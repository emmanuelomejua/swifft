        ...
        <SafeAreaView style={{ marginTop: Constants.statusBarHeight }}>
            <View style={tw`flex justify-start items-start gap-8 bg-white min-h-full p-4`}>
                <View style={tw`flex flex-row justify-start items-center gap-4`}>
                    ...
                    <Text style={tw.style({ fontFamily: 'PTSans-Regular' }, 'text-lg text-black')}>Edit Email Address</Text>
                </View>
                <View style={tw`flex justify-start items-start gap-4`}>
                    <Text style={tw.style({ fontFamily: 'PTSans-Bold' }, 'text-lg text-black')}>
                        Email Address
                    </Text>
                    <CustomInput onFocus={handleFocus} onBlur={handleBlur} value={number} onChangeText={(text) => setNumber(text)} placeholder="Enter Email Address" style={tw`px-6 py-4 min-w-full rounded-lg ${inputColor ? 'border-2 border-[#29BB00]' : 'border-2 border-slate-600'}`} />
                </View>
                <View style={tw`flex flex-row justify-center items-center mt-24`}>
                    <CustomButton
                        ...
                        onPress={handleModal}
                    >
                        <Text style={tw.style({ fontFamily: 'PTSans-Regular' }, `text-black text-center text-base ${inputColor ? 'text-white' : 'text-black'}`)}>Change</Text>
                    </CustomButton>
                    <CustomModal animationType={"slide"} transparent={true} visible={modalVisible} onRequestClose={() => setModalVisible(!modalVisible)}>
                        <View style={tw`flex justify-center items-center gap-4 w-72`}>
                            <Text style={tw.style({ fontFamily: 'PTSans-Bold' }, 'w-72 text-[#000000] text-xl text-center')}>Email Address Successfully Changed</Text>
                            <Text style={tw.style({ fontFamily: 'PTSans-Regular' }, 'text-[#8C8CA1] w-72 text-center text-base')}>You have successfully change your email address</Text>
